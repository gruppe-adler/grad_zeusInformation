...
private _error = switch (true) do {
    case (isNull _unit): {
        localize "str_a3_BIS_fnc_showCuratorFeedbackMessage_506"
    };
    case (isNull _grp || !(side _grp in [east,west,resistance,civilian])): {
        localize LSTRING(moduleToggleBFTError1)
    };
    case ({isPlayer _x} count (units _grp) > 0): {
        localize LSTRING(moduleBlacklistHeadlessError1)
    };
    default {""};
};
