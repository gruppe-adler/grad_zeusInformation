...
private _playersNumber = count _allPlayers;

switch (_updatePartID) do {

    // column 0 ================================================================
    case (0): {
        private _serverFPS = missionNamespace getVariable [QGVAR(serverFPS),-1];
        private _totalFPS = 0;
        private _playersCounted = 0;
        {
            private _totalFPS = 0;
        ...
        private _avgFPS = _totalFPS / (_playersCounted max 1);
    ...
    // column 1 ================================================================
    case (1): {
        private _unconsciousNumber = {_x getVariable ["ACE_isUnconscious",false]} count _allPlayers;
        private _unconsciousRatio = _unconsciousNumber/(_playersNumber max 1);
        private _injuredNumber = {!([_x] call ACE_medical_fnc_isInStableCondition)} count _allPlayers;
        private _injuredRatio = _injuredNumber/(_playersNumber max 1);
    ...
    case (3): {
        private _allAI = allUnits - _allPlayers;
        private _ownerNameArray = [];
        private _ownerCountArray = [];
        private _ownerArray = [];
        {
            private _ownerName = (group _x) getVariable [QGVAR(ownerName),"UNKNOWN"];
            private _ownerID = _ownerNameArray find _ownerName;
        ...
        if (count _ownerArray > 0) then {
            private _ownerData = _ownerArray select 0;
        ...
        if (count _ownerArray > 1) then {
            private _ownerData = _ownerArray select 1;
        ...
        if (count _ownerArray > 2) then {
            private _ownerData = _ownerArray select 2;
